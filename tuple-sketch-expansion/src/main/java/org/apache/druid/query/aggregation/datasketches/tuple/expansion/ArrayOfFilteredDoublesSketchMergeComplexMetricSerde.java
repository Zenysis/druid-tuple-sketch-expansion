...
public class ArrayOfFilteredDoublesSketchMergeComplexMetricSerde extends ComplexMetricSerde
{
  static final ObjectStrategy<ArrayOfDoublesSketch> STRATEGY = new ArrayOfDoublesSketchObjectStrategy()
  {
    @Override
    public int compare(final ArrayOfDoublesSketch s1, final ArrayOfDoublesSketch s2)
    {
      return ArrayOfFilteredDoublesSketchAggregatorFactory.COMPARATOR.compare(s1, s2);
    }
  };
  ...
  {
    return TupleSketchExpansionModule.ARRAY_OF_FILTERED_DOUBLES_SKETCH;
  }
  ...
  {
    final GenericIndexed<ArrayOfDoublesSketch> ge = GenericIndexed.read(
        buffer,
        STRATEGY
    );
    builder.setComplexColumnSupplier(
        new ComplexColumnPartSupplier(getTypeName(), ge)
    );
  }
